...
function sdtheme_posted_on() {
	...
	$posted_on = sprintf(
		...
		esc_html_x( 'Posted on %s', 'post date', 'sdtheme' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		/* translators: %s: post author. */
		esc_html_x( 'by %s', 'post author', 'sdtheme' ),
...
function sdtheme_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list ) {
			...
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'sdtheme' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
