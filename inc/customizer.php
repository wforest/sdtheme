 ...
 */
function sdtheme_customize_register( $wp_customize ) {
    ...
    $wp_customize->add_control(
      new WP_Customize_Color_Control(
          ...
          'theme_bg_color', array(
              'label'           => __( "Header and footer background color", 'sdtheme' ),
              'section'         => 'colors',
    ...
    $wp_customize->add_control(
        new WP_Customize_Color_Control(
            ...
            'interactive_color', array(
                'label'		=> __( 'Interactive color (links etc)', 'sdtheme' ),
                'section'	=> 'colors',
    ...
    $wp_customize->add_section( 'theme_options',
        array(
            'title'			=> __( 'Theme Options', 'sdtheme' ),
            'priority'		=> 95,
            'capability'	=> 'edit_theme_options',
            'description'	=> __( 'Change how much of a post is displayed on index and archive pages.', 'sdtheme' )
        )
    );
    ...
    $wp_customize->add_setting(	'length_setting',
        array(
            'default'			=> 'excerpt',
            'type'				=> 'theme_mod',
            'sanitize_callback' => 'sdtheme_sanitize_length', // Sanitization function appears further down
            'transport'			=> 'postMessage'
        )
    );

    // Add the controls
    $wp_customize->add_control(	'sdtheme_length_control',
        array(
            'type'		=> 'radio',
            'label'		=> __( 'Index/archive displays', 'sdtheme' ),
            'section'	=> 'theme_options',
            'choices'	=> array(
                'excerpt'		=> __( 'Excerpt (default)', 'sdtheme' ),
                'full-content'	=> __( 'Full content', 'sdtheme' )
            ),
            'settings'	=> 'length_setting' // Matches setting ID from above
        )
    );
}
add_action( 'customize_register', 'sdtheme_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function sdtheme_customize_preview_js() {
	wp_enqueue_script( 'sdtheme_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'sdtheme_customize_preview_js' );
 ...
 */
function sdtheme_sanitize_length( $value ) {
...
if ( ! function_exists( 'sdtheme_header_style' ) ) :
    /**
     * Styles the header image and text displayed on the blog.
     *
     * @see sdtheme_custom_header_setup().
     */
    function sdtheme_header_style() {
